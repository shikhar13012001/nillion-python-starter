...
async def main():
    ...
    for v in range(nr_voters):
        for c in range(nr_candidates):
            vote_key = f"v{v}_c{c}"
            role_key = f"role{v}_c{c}"
            secrets[vote_key] = nillion.SecretUnsignedInteger(max(v * 10 + c * 5, 1))
            secrets[role_key] = nillion.SecretUnsignedInteger(max(c + 1, 1))
        eligibility_key = f"eligibility{v}"
        secrets[eligibility_key] = nillion.SecretUnsignedInteger(1 if v % 2 == 0 else 1)
    ...
    for v in range(nr_voters):
        compute_bindings.add_input_party(f"Voter{v}", client.party_id)
